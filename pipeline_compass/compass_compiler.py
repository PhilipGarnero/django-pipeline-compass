...
class LibSassCompassCompiler(CompassCompiler):
    def compile_file(self, infile, outfile, outdated=False, force=False):
        ...
        if force or outdated:
            self.save_file(self.raw_path, sass.compile_file(str(infile), ','.join(self.include_paths), self.images_path))


def add_to_scss_path(paths):
...
class PyScssCompassCompiler(CompassCompiler):
    def compile_file(self, infile, outfile, outdated=False, force=False):
        ...
        add_to_scss_path(self.include_paths + [os.path.dirname(infile)])
        if force or outdated:
